...
export const PagePathNav = (props: Props): JSX.Element => {
  ...
  return (
    <div style={{ maxWidth }}>
      <span className={`small ${formerLinkClassName} ${styles['grw-former-link']}`}>{formerLink}</span>
...
export const PagePathNavSticky = (props: PagePathNavStickyProps): JSX.Element => {
  ...
  return (
    ...
    <div ref={pagePathNavRef}>
      <Sticky className={`${styles['grw-page-path-nav-sticky']} mb-5`} innerClass="mt-1 pe-none" innerActiveClass="active">
        {({ status }) => {
          ...
          //
            <div className="d-inline-block pe-auto">
              <PagePathNav
                ...
                formerLinkClassName={isCollapseParents ? '' : 'd-inline-block mb-sm-2'}
                latterLinkClassName={isCollapseParents ? 'fs-3  text-truncate' : ''}
